...
function SidePanel(props: Props) {
  return (
    <div className=" overflow-scroll fixed top-0 right-0 h-screen w-90 shadow-md bg-sidebar z-1001 px-4 py-8">
      <Suspense>
        <AirPollution {...props} />
      </Suspense>
    </div>
...
const getQualityColor = (level: AirQualityLevel) => {
  const colors = {
    Good: 'bg-green-500',
    Fair: 'bg-yellow-500',
    ...
    <div className="flex flex-col gap-4">
      ...
      <div className="flex items-center gap-2">
        ...
        <Tooltip>
          ...
          <TooltipContent className="z-2000 bg-white!">
      ...
      {Object.entries(data.list[0].components).map(([key, value]) => {
        ...
        const currentLevel = getAirQualityLevel(value, pollutant);

        const qualityColor = getQualityColor(currentLevel);
